...
public class TFMGPartialModels {
    ...
    public enum CogwheelMaterial {
        ALUMINUM,
        STEEL
    }

    public static @NotNull PartialModel cogwheel(boolean large, boolean shaftless, @NotNull CogwheelMaterial material) {
        switch (material) {
            case ALUMINUM -> {
                if(large) {
                    if(shaftless) {
                        return TFMGPartialModels.SHAFTLESS_LARGE_ALUMINUM_COGHWEEL;
                    } else {
                        return TFMGPartialModels.LARGE_ALUMINUM_COGHWEEL;
                    }
                } else {
                    if(shaftless) {
                        return TFMGPartialModels.SHAFTLESS_ALUMINUM_COGHWEEL;
                    } else {
                        return TFMGPartialModels.ALUMINUM_COGHWEEL;
                    }
                }
            }
            case STEEL -> {
                if(large) {
                    if(shaftless) {
                        return TFMGPartialModels.SHAFTLESS_LARGE_STEEL_COGHWEEL;
                    } else {
                        return TFMGPartialModels.LARGE_STEEL_COGHWEEL;
                    }
                } else {
                    if(shaftless) {
                        return TFMGPartialModels.SHAFTLESS_STEEL_COGHWEEL;
                    } else {
                        return TFMGPartialModels.STEEL_COGHWEEL;
                    }
                }
            }
        }
    ...
    public static @Nullable PartialModel cogwheel(boolean shaftless, Block block) {
        var material = cogwheelMaterial(block);
        if(material == null) {
            return null;
        }
        var large = cogwheelLarge(block);
        return cogwheel(large, shaftless, material);
    }

    public static boolean cogwheelLarge(Block block) {
        if(block == null) {
            return false;
        } else if(TFMGBlocks.ALUMINUM_COGWHEEL.is(block) ||
                TFMGBlocks.STEEL_COGWHEEL.is(block) ||
                TFMGEncasedBlocks.HEAVY_CASING_ENCASED_ALUMINUM_COGWHEEL.is(block) ||
                TFMGEncasedBlocks.HEAVY_CASING_ENCASED_STEEL_COGWHEEL.is(block) ||
                TFMGEncasedBlocks.STEEL_ENCASED_ALUMINUM_COGWHEEL.is(block) ||
                TFMGEncasedBlocks.STEEL_ENCASED_STEEL_COGWHEEL.is(block))
        {
            return false;
        } else if(
                TFMGBlocks.LARGE_ALUMINUM_COGWHEEL.is(block) ||
                TFMGBlocks.LARGE_STEEL_COGWHEEL.is(block) ||
                TFMGEncasedBlocks.HEAVY_CASING_ENCASED_LARGE_ALUMINUM_COGWHEEL.is(block) ||
                TFMGEncasedBlocks.HEAVY_CASING_ENCASED_LARGE_STEEL_COGWHEEL.is(block) ||
                TFMGEncasedBlocks.STEEL_ENCASED_LARGE_ALUMINUM_COGWHEEL.is(block) ||
                TFMGEncasedBlocks.STEEL_ENCASED_LARGE_STEEL_COGWHEEL.is(block))
        {
            return true;
        } else {
            return false;
    ...
    public static @Nullable CogwheelMaterial cogwheelMaterial(Block block) {
        if(block == null) {
            return null;
        } else if(TFMGBlocks.ALUMINUM_COGWHEEL.is(block) ||
                TFMGBlocks.LARGE_ALUMINUM_COGWHEEL.is(block) ||
                ...
                TFMGEncasedBlocks.STEEL_ENCASED_LARGE_ALUMINUM_COGWHEEL.is(block))
        {
            return CogwheelMaterial.ALUMINUM;
        } else if(TFMGBlocks.STEEL_COGWHEEL.is(block) ||
                TFMGBlocks.LARGE_STEEL_COGWHEEL.is(block) ||
                TFMGEncasedBlocks.HEAVY_CASING_ENCASED_STEEL_COGWHEEL.is(block) ||
                TFMGEncasedBlocks.HEAVY_CASING_ENCASED_LARGE_STEEL_COGWHEEL.is(block) ||
                TFMGEncasedBlocks.STEEL_ENCASED_STEEL_COGWHEEL.is(block) ||
